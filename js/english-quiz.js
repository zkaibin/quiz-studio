/**
 * English Quiz App - Standalone version for English questions
 */
...
class EnglishQuizApp {
  ...
  async initializeApp() {
    try {
      console.log('🚀 English Quiz initializing...');
      await this.loadEnglishData();
      await this.populateDropdowns();
      console.log('✓ English Quiz initialized');
    } catch (err) {
  ...
  async loadEnglishData() {
    try {
      ...
      const [questions, universes, characters] = await Promise.all([
        fetch(`data/questions-english.json?v=${cacheBuster}`).then(r => r.json()),
      ...
      console.log(`✓ Loaded ${this.allQuestions.length} English questions`);
      console.log(`✓ Loaded ${this.universes.length} themes`);
      console.log(`✓ Loaded ${this.characters.length} characters`);
    } catch (err) {
      console.error('❌ Error loading English data:', err);
      this.allQuestions = [];
      this.universes = [];
      this.characters = [];
    }
  ...
  async startQuiz() {
    ...
    if (window.QuizPersonalization && typeof window.QuizPersonalization.getPrioritizedQuestions === 'function') {
      allQuestions = await window.QuizPersonalization.getPrioritizedQuestions('english', allQuestions);
    } else {
...
document.addEventListener('DOMContentLoaded', () => {
  window.englishQuizApp = new EnglishQuizApp();
});
